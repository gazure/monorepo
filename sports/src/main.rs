...
#[derive(Parser)]
#[command(name = "sports")]
#[command(about = "Baseball Reference box score parser and database importer")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::ListGames { schedule, limit } => {
        ...
        Commands::RetryImports {
            ...
            for (i, file_path) in files.iter().enumerate() {
                ...
                match inserter.insert(&box_score).await {
                    ...
                    Err(sports::db::InsertError::GameExists(_)) => {
                        println!(
                            "✓ {} (already exists)",
                            file_path.file_name().unwrap().to_string_lossy()
                        );
